...
from prune_utils import PruneConfig
from nlu_modelling import RobertaForNLU, RobertaNLUConfig, MASK_VALUE
...
def parse_args(raw_args=None):
    ...
    parser.add_argument("--prune_avg_window_size", type=int, default=1, help="Number of reported scores before 'prune_epoch' to average on for 'prune_avg_mode' == 'avg'.")
    ...
    parser.add_argument("--dataset", type=str, default="atis", help="Dataset to train and evaluate on.")
...
def generate_dataset(dataset_name: str, tokenizer, random_seed: int = 1234):
    path = f'datasets/{dataset_name}/nlu_data'
    dataset = load_dataset("parquet", data_files={'train': f'{path}/train.parquet', 'test': f'{path}/test.parquet'})
...
def main(raw_args=None):
    ...
    dataset_name = args_in.dataset
    assert dataset_name in {"atis", "slurp", "snips", "mtop", "nlu_eval"}, "dataset needs to be in datasets folder."
    ...
    train, test, id2intent, id2slots = generate_dataset(dataset_name, tokenizer, random_seed)
